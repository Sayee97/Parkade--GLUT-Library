...
int
c[36]={159,243,133,253,233,228,141,18,46,195,75,52,253,204,169,30,78,94,68,117
...
glVertex3f(-1.12,-.48,0.7);//a   /////////////////////FACE 1 car 18 points
...
glEnd();
glBegin(GL_LINE_LOOP);
glVertex3f(-1.12,-.48,-0.7);//a' ///////////////Face 2 CAr 18 points
glVertex3f(-0.86,-.48,-0.7);//b'
glVertex3f(-.74,-0.2,-0.7);//c'
glVertex3f(-.42,-.2,-0.7);//d'
glVertex3f(-0.3,-.48,-0.7);//e'
...
glEnd();
glBegin(GL_LINES);
glVertex3f(-1.12,-.48,0.7);//a  ////////////////////// 36 points car joining skeleton
...
glVertex3f(-1.2,-.28,-0.7);//r'
glEnd();
glBegin(GL_POLYGON); // top filling ///////////////////////FILLING
 glVertex3f(-0.1,0.6,0.7);//o
glVertex3f(-0.1,0.6,-0.7);//o'
glVertex3f(-1.02,0.6,-0.7);//p'
glVertex3f(-1.02,0.6,0.7);//p
...
glVertex3f(-1.12,-.48,0.7);//a
glVertex3f(-0.86,-.48,0.7);//b
glVertex3f(-.74,-0.2,0.7);//c ////////////////////// Keeping Z constant ..for curve we are increasing X and Y
...
//range	 Specifies the number of contiguous empty display
...
glColor3ub(a[i],b[j],c[i]);    /////glubyte me color
glCallList(carr_display_list); //glCallList — execute a display list
...
z = z + i*(lz)*0.5;
 x = x + i*(lx)*0.5;}
glLoadIdentity();  //ut if you want to start over at the origin, that's when you call glLoadIdentity()
gluLookAt(x, y, z,x + lx,y + ly,z + lz,0.0f,1.0f,0.0f);
}
...
switch (key)
{
case GLUT_KEY_LEFT :temp=fxincr;//fxincr=0.1 //fz=0
...
glutIdleFunc(renderScene); //  glutIdleFunc sets the global idle callback.
